...
export function LembagaRow({ item }: { item: Lembaga }) {
      ...
      <tr className="bg-surface-alt">
        <td colSpan={6} className="px-3 py-3">
          <form action={formAction} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
            ...
            <div>
              <label className="text-xs text-text-muted">Dasar Hukum</label>
              <Input name="dasar_hukum" defaultValue={item.dasar_hukum ?? ""} maxLength={120} />
            </div>
            <div>
              <label className="text-xs text-text-muted">Jumlah Pengurus</label>
              <Input
  ...
  return (
    <tr className="border-b border-border last:border-0">
      ...
      <td className="px-3 py-2.5 text-sm text-text-muted">{item.dasar_hukum ?? "—"}</td>
